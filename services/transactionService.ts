  ...
  try {
    ...
    if (id) {
      ...
      if(shouldRevertOrginal) {
        let res  = await revertAndUpdateWallets(oldTransacton,Number(amount),type,walletId)
...
const revertAndUpdateWallets = async (
  ...
  try {
    const orginalWalletSnapshot = await getDoc(doc(firestore, "wallets", oldTransaction.walletId)); 
    ...
    const revertedIncomeExpenseAmount = Number(orginalWallet[revertType]) - Number(oldTransaction.amount)
  ...
  const updatedType = newTransactionType == "income" ? "totalIncome" : "totalExpenses";
